...
use crate::poly::opening_proof::{OpeningPoint, ProverOpeningAccumulator, SumcheckId, LITTLE_ENDIAN};
...
impl<F: JoltField> BaselineSpartanInterleavedPolynomial<F> {
  ...
  /// Returns (0, tinf) since cz terms do not contribute in the first round.
  pub fn endpoints_unbound_first_round(
      &self,
      ...
      let quadratic_eval_at_infty = chunks
          ...
          .map(|chunk| {
              ...
              for sparse_block in chunk.chunk_by(|x, y| x.index / 6 == y.index / 6) {
                  ...
                  inner_sum += E_in_eval * (az_infty * bz_infty);
              }
  ...
  pub fn endpoints_bound_remaining(
      &self,
      eq_poly: &GruenSplitEqPolynomial<F>,
  ) -> (F, F) {
      let block_size = self
...
pub struct OuterBaselineSumcheckProver<F: JoltField> {
  ...
  eq_poly: GruenSplitEqPolynomial<F>,
  /// Interleaved Az/Bz/Cz holder and binding workspace
