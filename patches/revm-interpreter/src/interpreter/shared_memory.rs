...
impl MemoryTr for SharedMemory {
    ...
    fn set_u256_be(&mut self, memory_offset: usize, value: &U256) {
        self.set_u256_be(memory_offset, value);
    }
...
impl SharedMemory {
    ...
    /// Sets the given U256 value to memory in canonical big-endian byte order.
    ///
    /// This keeps the byte semantics identical to `to_be_bytes::<32>()`, but
    /// when the destination is 8-byte aligned it writes four u64 limbs
    /// directly instead of routing through a byte-slice copy.
    #[inline]
    #[cfg_attr(debug_assertions, track_caller)]
    pub fn set_u256_be(&mut self, offset: usize, value: &U256) {
        let mut dst = self.slice_mut(offset, 32);
        let ptr = dst.as_mut_ptr();
        if ptr.align_offset(core::mem::align_of::<u64>()) == 0 {
            let words = ptr.cast::<u64>();
            let limbs = value.as_limbs();
            unsafe {
                // SAFETY: `slice_mut(offset, 32)` guarantees 32 writable bytes.
                // The alignment check above guarantees `ptr` is valid for
                // `u64` stores. `U256` stores little-endian limbs, so reversing
                // limb order and byte-swapping each one preserves the expected
                // big-endian memory layout.
                words.add(0).write(limbs[3].to_be());
                words.add(1).write(limbs[2].to_be());
                words.add(2).write(limbs[1].to_be());
                words.add(3).write(limbs[0].to_be());
            }
        } else {
            dst.copy_from_slice(&value.to_be_bytes::<32>());
        }
    }
