...
impl<F: JoltField> G2AddValues<F> {
    ...
    ///
    /// Fq2 arithmetic is done component-wise:
    /// - Addition: (a0,a1) + (b0,b1) = (a0+b0, a1+b1)
    /// - Multiplication: (a0,a1) * (b0,b1) = (a0*b0 - a1*b1, a0*b1 + a1*b0)
    /// - Square: (a0,a1)^2 = (a0^2 - a1^2, 2*a0*a1)
    pub fn eval_constraint(&self, delta: F) -> F {
        ...
        let two_yp_lam_c0 = two * mul_c0(self.y_p_c0, self.y_p_c1, self.lambda_c0, self.lambda_c1);
        let two_yp_lam_c1 = two * mul_c1(self.y_p_c0, self.y_p_c1, self.lambda_c0, self.lambda_c1);
        let three_xp_sq_c0 = three * sq_c0(self.x_p_c0, self.x_p_c1);
        let three_xp_sq_c1 = three * sq_c1(self.x_p_c0, self.x_p_c1);
