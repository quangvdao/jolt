...
pub trait MemoryTr {
    ...
    /// Sets a 32-byte U256 value in big-endian byte order at the given
    /// memory offset.
    ///
    /// The default implementation falls back to a temporary byte array so
    /// custom memory backends can ignore it. Optimized backends can
    /// override this to write whole limbs directly.
    fn set_u256_be(&mut self, memory_offset: usize, value: &U256) {
        self.set(memory_offset, &value.to_be_bytes::<32>());
    }
