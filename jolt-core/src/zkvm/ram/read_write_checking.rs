...
impl<F: JoltField> RamReadWriteCheckingProver<F> {
    ...
    pub fn gen(
        ...
        let sparse_matrix = SparseMatrixPolynomial::new(&trace, &preprocessing.memory_layout);
    ...
    fn phase1_compute_prover_message(&mut self, previous_claim: F) -> Vec<F> {
        let Self {
            inc,
            eq_r_prime,
            params,
...
impl<F: JoltField, T: Transcript> SumcheckInstanceProver<F, T> for RamReadWriteCheckingProver<F> {
    ...
    fn compute_prover_message(&mut self, round: usize, previous_claim: F) -> Vec<F> {
        if round < self.params.T.log_2() {
            self.phase1_compute_prover_message(previous_claim)
        } else {
