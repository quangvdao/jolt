...
impl<F: JoltField> NewSpartanInterleavedPolynomial<F> {
    ...
    pub fn new_with_precompute(
        ...
        // Currently copied over from `SpartanInterleavedPolynomial::new`
    ...
    pub fn streaming_sumcheck_round<ProofTranscript: Transcript>(
        ...
        // 2. Compute dense bound evaluations for current variable = 0 and 1
        // These vectors will hold P(r_{[<i]}, u, x') where u=0 or u=1
        ...
        let mut cz_evals_1: Vec<F> = vec![F::zero(); N_k]; // Needed for Cz binding later

        // TODO(Optimize): Parallelize this sparse-to-dense accumulation carefully.
        // Simple sequential version for now:
        for coeff in &self.unbound_coeffs {
            // The index in unbound_coeffs corresponds to the *original* dense polynomial index * 3 + comp
            let dense_idx = coeff.index / 3; // Index into the effective dense vector of size 2^total_vars
            let comp_idx = coeff.index % 3;  // 0 for Az, 1 for Bz, 2 for Cz

            // Deconstruct dense_idx into parts based on current_var_index_k
            let x_prime_idx = dense_idx & (N_k - 1); // Lower k bits (indices 0..k-1)
            let y_k = (dense_idx >> current_var_index_k) & 1; // Bit k (current variable's value in original poly)
            let y_high_idx = dense_idx >> (current_var_index_k + 1); // Higher bits (indices k+1..ell-1)

            if y_high_idx >= eq_r.len() { continue; } // Should not happen if dimensions match

            // Contribution to P(r_{high}, y_k, x') = eq(r_{high}, y_{high}) * P(y_{high}, y_k, x')
            let term = eq_r[y_high_idx] * F::from_i128(coeff.value);

            // Accumulate into the vectors representing P(r_{high}, u, x') for u=0 and u=1
            match comp_idx {
                0 => { // Az
                    if y_k == 0 { az_evals_0[x_prime_idx] += term; } 
                    else { az_evals_1[x_prime_idx] += term; }
                }
                1 => { // Bz
                    if y_k == 0 { bz_evals_0[x_prime_idx] += term; } 
                    else { bz_evals_1[x_prime_idx] += term; }
                }
                2 => { // Cz
                    if y_k == 0 { cz_evals_0[x_prime_idx] += term; } 
                    else { cz_evals_1[x_prime_idx] += term; }
                }
                _ => unreachable!(),
            }
        }
        ...
        //    The logic depends on eq_poly's state (E1_len) and sums over x' (indices 0..N_k-1)
        let (quadratic_eval_at_0, quadratic_eval_at_infty) = if eq_poly.E1_len() == 1 {
             ...
             debug_assert_eq!(eq_poly.E2_len(), N_k, "E2_len should match N_k when E1_len is 1");
             // Sum over all N_k points, weighted only by E2
             (0..N_k).into_par_iter()
                 .map(|idx_rest| { // idx_rest corresponds to x'
             ...
             debug_assert_eq!(N_k, num_e2_points * num_e1_points, "N_k should be product of E1_len and E2_len");

             eq_poly.E2_current().par_iter().enumerate()
                 .map(|(e2_idx, e2_val)| { // e2_idx is index over x_out
                     let mut inner_sum_eval_at_0 = F::zero();
                     let mut inner_sum_eval_at_infty = F::zero();
                     let start_idx_rest = e2_idx * num_e1_points;

                     for e1_idx in 0..num_e1_points { // e1_idx is index over x_in
                         ...
                         let term_eval_at_0 = az0 * bz0 - cz0; // P(r_{high}, 0, x_out, x_in)
                         let term_eval_at_infty = az_m * bz_m; // P(r_{high}, infty, x_out, x_in)
                     ...
                     (*e2_val * inner_sum_eval_at_0, *e2_val * inner_sum_eval_at_infty)
                 })
                 .reduce(|| (F::zero(), F::zero()), |a, b| (a.0 + b.0, a.1 + b.1))
        };
        ...
        az_bound_next.par_iter_mut().enumerate().for_each(|(idx, val)| {
             *val = az_evals_0[idx] + r_i * (az_evals_1[idx] - az_evals_0[idx]);
        });
        bz_bound_next.par_iter_mut().enumerate().for_each(|(idx, val)| {
             *val = bz_evals_0[idx] + r_i * (bz_evals_1[idx] - bz_evals_0[idx]);
        });
        cz_bound_next.par_iter_mut().enumerate().for_each(|(idx, val)| {
        ...
        self.unbound_coeffs = Vec::new(); // Free memory
    ...
    pub fn remaining_sumcheck_round<ProofTranscript: Transcript>(
        ...
        let (quadratic_eval_at_0, quadratic_eval_at_infty) = if eq_poly.E1_len() == 1 {
            ...
            debug_assert_eq!(eq_poly.E2_len(), num_x_rest_evals, "E2_len should match num_x_rest_evals when E1_len is 1");
            ...
            az_low.par_iter()
                ...
                .map(|(((((az0_ref, az1_ref), bz0_ref), bz1_ref), cz0_ref), e2_val_ref)| {
                    let az0 = *az0_ref; let az1 = *az1_ref;
                    let bz0 = *bz0_ref; let bz1 = *bz1_ref;
                    let cz0 = *cz0_ref; let e2_val = *e2_val_ref;
                ...
                .reduce(
                    || (F::zero(), F::zero()),
                    |(acc_0, acc_infty), (val_0, val_infty)| {
                        (acc_0 + val_0, acc_infty + val_infty)
                    },
                )
        } else {
            ...
            debug_assert_eq!(num_x_rest_evals, num_e2_points * num_e1_points, "num_x_rest_evals should be product of E1_len and E2_len");
