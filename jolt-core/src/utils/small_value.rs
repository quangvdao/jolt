    ...
    pub fn fmadd_unreduced<F: JoltField>(
        ...
        if !product.is_zero() {
            let mag: <F as JoltField>::Unreduced<4> =
                <F as JoltField>::Unreduced::from(product.magnitude_as_bigint_nplus1());
            let term = field_bigint.mul_trunc::<4, 8>(&mag);
        ...
        pub fn fmadd_bz(&mut self, field: &F, bz: S160) {
            ...
            if !bz.is_zero() {
                let mag: <F as JoltField>::Unreduced<3> =
                    <F as JoltField>::Unreduced::from(bz.magnitude_as_bigint_nplus1());
                let term = field_bigint.mul_trunc::<3, 8>(&mag);
                if bz.is_positive() {
                    self.pos += term;
                } else {
                    self.neg += term;
