...
pub struct MultiMillerLoopSteps {
    ...
    /// Per-pair Miller loop outputs (in Fq12, before final exponentiation).
    pub pair_results: Vec<Fq12>,
...
impl MultiMillerLoopSteps {
    pub fn new(g1s: &[G1Affine], g2s: &[G2Affine]) -> Self {
        ...
        for (pair_idx, (&p, &q)) in g1s.iter().zip(g2s.iter()).enumerate() {
            ...
            pair_results.push(trace.final_f);
...
fn trace_single_pair(p: G1Affine, q: G2Affine) -> SinglePairTrace {
    ...
    f_step_mles.push(terminal_f_mle.clone());
    // For shift consistency, keep `f_next` equal to the next-step `f` on inactive rows.
    f_next_step_mles.push(terminal_f_mle.clone());
    quotient_step_mles.push(vec![Fq::zero(); ELEM_SIZE]);
    l_val_step_mles.push(vec![Fq::zero(); ELEM_SIZE]);

    t_x.push(t.x);
    t_y.push(t.y);
    // For shift consistency, keep `T_next` equal to the next-step `T` on inactive rows.
    t_x_next.push(t.x);
    t_y_next.push(t.y);
    ...
    // Pad out to the full 7-bit step domain so the final output always lives at s=127.
    //
    // This matches the shift gadget's "not_last" masking at s=127 and avoids needing to expose
    // `num_steps` to the verifier.
    let mut num_packed_steps = num_steps + 1; // include terminal row
    while num_packed_steps < STEP_SIZE {
        f_step_mles.push(terminal_f_mle.clone());
        f_next_step_mles.push(terminal_f_mle.clone());
        quotient_step_mles.push(vec![Fq::zero(); ELEM_SIZE]);
        l_val_step_mles.push(vec![Fq::zero(); ELEM_SIZE]);
    ...
    debug_assert_eq!(num_packed_steps, STEP_SIZE);
