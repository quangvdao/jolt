...
impl<F: JoltField> NewSpartanInterleavedPolynomial<F> {
    ...
    pub fn streaming_sumcheck_round<ProofTranscript: Transcript>(
        ...
        claim: &mut F,)  {
        // Using `unbound_coeffs` to compute the evals {Az/Bz/Cz}(r, u, x') needed for later linear-time
        // sumcheck rounds
        // And of course, compute the polynomial of this round
        ...
        let E1_evals: Vec<_> = eq_poly.E1_current()
            .par_chunks(2)
            .map(|E1_chunk| {
            ...
            .map(|chunk| {
                ...
                for sparse_block in chunk.chunk_by(|x, y| x.index / 6 == y.index / 6) {
                    ...
                    if x2 != prev_x2 {
                        ...
                        eval_point_infty += eq_poly.E2[prev_x2] * inner_sums.1;
