...
impl<F: JoltField, ProofTranscript: Transcript> SumcheckInstanceProof<F, ProofTranscript> {
    ...
    /// Version of `prove_spartan_small_value` that uses Gruen's optimization (but no small value optimization)
    #[tracing::instrument(skip_all, name = "Spartan::prove_spartan_with_gruen")]
    pub fn prove_spartan_with_gruen(
        num_rounds: usize,
        padded_num_constraints: usize,
        uniform_constraints: &[Constraint],
        flattened_polys: &[MultilinearPolynomial<F>],
        ...
        // Create Gruen polynomial and eq poly
        let mut az_bz_cz_poly = GruenSpartanInterleavedPolynomial::<F>::new(
            uniform_constraints,
            flattened_polys,
            padded_num_constraints,
        );
