...
const CYCLE_VERIFY_RECURSION_STAGE2_G1_WIRING_TOTAL: &str =
    "verify_recursion_stage2_g1_wiring_total";
...
pub struct WiringG1Prover<T: Transcript> {
    ...
    pairing_p2: (Fq, Fq, Fq),
    pairing_p3: (Fq, Fq, Fq),
    // GT-derived / proof-derived G1 inputs, batched as V = x + μ·y + μ^2·ind.
    g1_inputs_batched: BTreeMap<u32, Fq>,
...
impl<T: Transcript> WiringG1Prover<T> {
    pub fn new(
        ...
        let g1_inputs_batched: BTreeMap<u32, Fq> = g1_inputs
            .iter()
            .map(|(value_id, p)| {
                let (x, y, ind) = g1_const_from_affine(p);
                let v = x + mu * y + mu2 * ind;
                (*value_id, v)
            })
            .collect();
        ...
        match v {
            ...
            G1ValueRef::G1Input { value_id } => {
                let v = *self
                    .g1_inputs_batched
                    .get(&value_id)
                    .expect("missing g1_inputs_batched entry for G1Input");
                [v; DEGREE]
            }
        }
    }
...
pub struct WiringG1Verifier {
    ...
    ///
    /// Cached so `expected_output_claim` can precompute `Eq(c_tail, idx)` once per instance.
    num_smul_instances: usize,
    /// Number of G1Add instances referenced by the wiring plan.
    ...
    /// Boundary G1 inputs (AST `ValueId.0` -> V = x + μ·y + μ^2·ind).
    g1_inputs_batched: BTreeMap<u32, Fq>,
}
...
impl WiringG1Verifier {
    pub fn new<T: Transcript>(input: &RecursionVerifierInput, transcript: &mut T) -> Self {
        ...
        let smul_base_boundary = input
            ...
            .collect();
        let g1_inputs_batched: BTreeMap<u32, Fq> = input
            .g1_inputs
            .iter()
            .map(|(value_id, p)| {
                let (x, y, ind) = g1_const_from_affine(p);
        ...
        // Cache how many family instances are referenced by the edge list (avoid re-scanning or
        // recomputing Eq(c_tail, idx) per edge).
...
impl<T: Transcript> SumcheckInstanceVerifier<Fq, T> for WiringG1Verifier {
    fn cycle_tracking_label(&self) -> Option<&'static str> {
        Some(CYCLE_VERIFY_RECURSION_STAGE2_G1_WIRING_TOTAL)
    }
    ...
    fn expected_output_claim(
        ...
        for (lambda, edge) in self.lambdas.iter().zip(self.edges.iter()) {
            ...
            let (b_src, eqc_src, v_src) = match edge.src {
                ...
                | G1ValueRef::PairingBoundaryP2
                | G1ValueRef::PairingBoundaryP3
                | G1ValueRef::G1Input { .. } => (b_dst, eqc_dst, v_src),
            ...
            let (b_dst, eqc_dst, v_dst) = match edge.dst {
                ...
                | G1ValueRef::PairingBoundaryP2
                | G1ValueRef::PairingBoundaryP3
                | G1ValueRef::G1Input { .. } => (b_src, eqc_src, v_dst),
