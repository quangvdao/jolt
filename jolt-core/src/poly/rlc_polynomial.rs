...
impl<F: JoltField> RLCPolynomial<F> {
    ...
    fn materialized_vector_matrix_product(
        ...
        // Precompute per-row factors and eq_k (size K) from the Dory left vector, then
        // fold γ-coefficients into K-sized per-polynomial tables.
        ...
        let (dense_accs, onehot_accs) = chunk_ranges
            ...
            .map(|(row_start, row_end)| {
                ...
                for row_idx in row_start..row_end {
                    ...
                    // Debug assertions for padding (NoOp cycles).
                    #[cfg(test)]
