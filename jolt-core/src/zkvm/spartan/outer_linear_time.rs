...
impl<F: JoltField> GruenSpartanInterleavedPolynomial<F> {
    pub fn new(
        ...
        let unbound_coeffs_shards: Vec<Vec<SparseCoefficient<i128>>> = (0..num_chunks)
            .into_par_iter()
            .map(|chunk_index| {
                let step_start = chunk_size * chunk_index;
                let step_end = std::cmp::min(chunk_size * (chunk_index + 1), num_steps);

                let mut coeffs =
                    Vec::with_capacity((step_end - step_start) * padded_num_constraints * 2);
    ...
    pub fn first_sumcheck_round<ProofTranscript: Transcript>(
            ...
            .map(|shard_coeffs| {
                ...
                for sparse_block in shard_coeffs.chunk_by(|x, y| x.index / 6 == y.index / 6) {
                    ...
                    if az_infty != 0 && bz_infty != 0 {
                        current_shard_inner_sums +=
                            E_in_evals.mul_i128_1_optimized(az_infty.wrapping_mul(bz_infty));
                    }
                }
        ...
        let output_sizes: Vec<_> = self
            .unbound_coeffs_shards
            .par_iter()
            .map(|shard| Self::binding_output_length(shard.as_ref()))
            .collect();
        ...
        let quadratic_eval_at_infty = chunks
            ...
            .map(|chunk| {
                ...
                for sparse_block in chunk.chunk_by(|x, y| x.index / 6 == y.index / 6) {
                    ...
                    if x_out != prev_x_out {
                        eval_point_infty += eq_poly.E_out_current()[prev_x_out] * inner_sum;
                        inner_sum = F::zero();
                        prev_x_out = x_out;
                    }
                    ...
                    if az_infty != 0 && bz_infty != 0 {
                        inner_sum +=
                            E_in_eval.mul_i128_1_optimized(az_infty.wrapping_mul(bz_infty));
                    }
                }
