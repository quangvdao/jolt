...
pub trait StreamingMLE<F: JoltField>: Send {
    ...
    fn sumcheck_evals<const DEGREE: usize>(&self, i: usize) -> [F; DEGREE];
}
...
impl<F: JoltField> RlcStreamingPolynomial<F> {
    ...
    pub fn sumcheck_evals<const DEGREE: usize>(&self, i: usize) -> [F; DEGREE] {
        let mut acc = [F::zero(); DEGREE];
        for (c, s) in self.coeffs.iter().zip(self.streams.iter()) {
            let evals = s.sumcheck_evals::<DEGREE>(i);
            for j in 0..DEGREE {
                acc[j] += *c * evals[j];
            }
        }
