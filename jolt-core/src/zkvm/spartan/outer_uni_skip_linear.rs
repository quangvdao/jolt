...
#[derive(Allocative)]
pub struct OuterRemainingSumcheckProverNonStreaming<F: JoltField> {
    #[allocative(skip)]
    bytecode_preprocessing: BytecodePreprocessing,
    #[allocative(skip)]
    trace: Arc<Vec<Cycle>>,
    ...
    #[allocative(skip)]
    params: OuterRemainingSumcheckParamsNonStreaming<F>,
}

impl<F: JoltField> OuterRemainingSumcheckProverNonStreaming<F> {
    #[tracing::instrument(skip_all)]
    pub fn gen(
        trace: Arc<Vec<Cycle>>,
        bytecode_preprocessing: &BytecodePreprocessing,
...
impl<F: JoltField, T: Transcript> SumcheckInstanceProver<F, T>
    for OuterRemainingSumcheckProverNonStreaming<F>
{
    fn degree(&self) -> usize {
    ...
    fn cache_openings(
        ...
        let r_cycle =
            OuterRemainingSumcheckParamsNonStreaming::get_inputs_opening_point(sumcheck_challenges);
...
pub struct OuterRemainingSumcheckVerifierNonStreaming<F: JoltField> {
    params: OuterRemainingSumcheckParamsNonStreaming<F>,
...
impl<F: JoltField, T: Transcript> SumcheckInstanceVerifier<F, T>
    for OuterRemainingSumcheckVerifierNonStreaming<F>
{
    fn degree(&self) -> usize {
        OUTER_REMAINING_DEGREE_BOUND
    }
...
pub(crate) struct OuterRemainingSumcheckParamsNonStreaming<F: JoltField> {
...
impl<F: JoltField> OuterRemainingSumcheckParamsNonStreaming<F> {
    ...
    /// Returns the opening point for all R1CS input polynomials at `r_cycle`.
    pub(crate) fn get_inputs_opening_point(
        sumcheck_challenges: &[F::Challenge],
